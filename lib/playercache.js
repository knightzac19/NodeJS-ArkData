            ...
            checkId(item.Id, function(d) {
                if (d === undefined && item.Id !== false) {
                    db.parallelize(function() {
                        db.run("INSERT INTO players (id,steamid,charactername,level,engrams,tribeid,last_updated) VALUES (?,?,?,?,?,?,?)", [item.Id, item.SteamId, item.PlayerName, item.Level, item.TotalEngramPoints,item.TribeId, item.FileUpdated], function(err, sql) {
                            if (err) {
...
module.exports.setupPlayers = (cb) => {
    ...
    db.parallelize(function() {
        db.run('CREATE TABLE IF NOT EXISTS players (Id INTEGER  NOT NULL UNIQUE, TribeId INT NULL, Level INT NOT NULL, Engrams INT NOT NULL, SteamId VARCHAR NOT NULL UNIQUE, Admin BOOL FALSE, CharacterName VARCHAR NULL,	SteamName VARCHAR NULL,ProfileUrl VARCHAR NULL,	AvatarUrl VARCHAR NULL,	CommunityBanned INT NULL,	VACBanned INT NULL,	NumberOfVACBans INT NULL,	NumberOfGameBans INT NULL,	DaysSinceLastBan INT NULL,	last_updated DATETIME NULL)');
