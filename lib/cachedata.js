...
let tribeOwners = [];
let tribeMembers = [];
let rootPath = "D:\\Steam\\steamapps\\common\\ARK\\ShooterGame";
    ...
    files.forEach(f => {
      ...
      if (f.split(".")[1] == "arkprofile") {
        ...
        let tribeMemberObject = {
          ...
          TribeId: parse.getInt("TribeID", contents)
        };
...
function loadSteam(list) {
  ...
  return new Promise(function(r, rj) {
    Steam.key = '71F586341F08443E86B3AEA822684952';
    Steam.ready((err) => {
      if (err) {
        rj(err);
      }
