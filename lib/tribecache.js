            ...
            checkId(item.Id, function(d) {
                if (d === undefined && item.Id !== false) {
                    db.parallelize(function() {
                        db.run("INSERT INTO tribes (Id,Name,OwnerId,CreateTime,LastUpdated ) VALUES (?,?,?,?,?)", [item.Id, item.Name, item.OwnerId, item.FileCreated, item.FileUpdated], function(err, sql) {
...
module.exports.setupTribes = function(cb) {
    db.serialize(function() {
        db.run('CREATE TABLE IF NOT EXISTS tribes ( Id INTEGER  NOT NULL UNIQUE,Name VARCHAR NOT NULL,OwnerId INT NULL,	CreateTime DATETIME NULL,LastUpdated DATETIME NULL)');
